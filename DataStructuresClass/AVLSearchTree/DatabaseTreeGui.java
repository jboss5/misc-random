...
public class DatabaseTreeGui extends JFrame
{
	...
	private static AVLTree avl; //Holding a reference to the binary search tree.
    ...
    private boolean isValidOrderDate(String date)
    {
    	...
    	else
    	{
    		...
    		int orderYear = Integer.parseInt(dateInpString[2]); //The user inputed year.
	    	
    		if(orderDay < today && orderYear <= year)
	    		return true;
    		else if(orderMonth < month && orderYear <= year)
	    		return true;
			else
	    		return false;
    	}
    ...
    private boolean isValidDueDate(String date)
    {
    	...
    	else
    	{
			...
			if(validDay > endOfMonth && orderYear >= year)
			{
				validDay -= endOfMonth;
				if(dueDay >= validDay)
					return true;
    		...
    		else if(dueDay >= validDay && dueMonth >= month || orderYear > year)
    			return true;
    		else if(dueDay < validDay && dueMonth > month && orderYear >= year)
    			return true;
			else
    ...
    private void delRecord(JFrame gui, JTextField num)
    {
    	...
    	Object find = avl.find(inpNum); //Find the order number.
    	...
    	else //If it is found.
    	{
    		...
    		if(option == 0)
    		{   
    			boolean del = avl.delete(order.getNumber()); //Determines whether the delete was successful or not.
    ...
    private void findRecord(JFrame gui, JTextField num)
    {
    	...
    	Object find = avl.find(inpNum); //Finds the order number.
    	if(find == null) //If its not found.
    	{
    		JOptionPane.showMessageDialog(gui, 
    ...
    private static void buildOrderArray(AVLTree avl, Order[] orderArray, Scanner database)
    {
    	...
    	while(database.hasNext())
    	{
    		...
    		Index index = new Index(orderNum, ind); //New index to add to a new node of the AVL tree.
    		
    		avl.insert(index, orderNum);
    		ind++;
    	}
    ...
    public static void main(String[] args)
    {
    	...
    	avl = new AVLTree(); //Initializing the AVL tree.
    	...
    	try
    	{
    		...
    		buildOrderArray(avl, orderArray, database); //Builds the order array at the start of the program.
    	}
    	catch(FileNotFoundException e)
    	{
